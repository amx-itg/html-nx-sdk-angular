...
@Component({
  selector: 'amx-nx-button',
  templateUrl: './amx-nx-button.component.html',
  styleUrls: ['./amx-nx-button.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class AmxNxButtonComponent implements OnDestroy, OnChanges {
  ...
  bindEvent(): void {
    this.channelEventSubscription = this.eventBusService.on('channel.event').subscribe((data) => {
      if (data && data.port) {
        const btn = Object.assign({}, this.btn);
